# Professional Positioning

## One-line positioning

Senior SAP IBP and manufacturing planning consultant, helping European manufacturers connect SAP IBP, S/4HANA PP, and Digital Manufacturing into a practical, compliant planning-to-execution process.

## Core focus

- SAP IBP: S&OP, demand planning, supply planning, and related inventory concepts
- SAP S/4HANA: PP, planning and logistics processes, automotive and discrete manufacturing
- SAP Digital Manufacturing: process understanding to bridge planning and shop-floor discussions
- Management of Change and Engineering Change Management: workflows aligned to audit and compliance needs

## Niche

I specialise in projects where:

- Planning and manufacturing need to be aligned across IBP, PP, and production execution
- CI-DS integration between SAP IBP and S/4HANA must be designed and stabilised
- Regulatory or audit requirements (e.g. FDA, CAPA) demand structured change and approval workflows
- Clients need a consultant who can both design processes and train internal teams at scale

## Differentiators

- Hands-on integration experience between SAP IBP and S/4HANA via CI-DS in multi-plant rollouts
- Proven impact on change-cycle efficiency and CAPA closure through MOC and ECM workflow design
- Experience as SAP Academy trainer and programme lead across multiple countries and cohorts
- Ability to explain complex SAP planning/manufacturing topics in language that business stakeholders understand

## Availability and engagement model

- Remote-first engagements across the EU, based in Greece
- Contract roles of 3–6+ months with minimal travel
- Daily rate positioned in the mid–senior freelance band, aligned with project scope and responsibility
